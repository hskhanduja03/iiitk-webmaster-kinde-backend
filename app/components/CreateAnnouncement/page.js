...
const AnnouncementManager = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const announcementResponse = await fetch('/api/announcements', { method: 'GET' });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const payload = { title, description, date, imageurl, link };
      ...
      <div>
        ...
        {announcements.length > 0 ? (
          announcements.map((announcement) => (
            <div key={announcement._id} className="border p-4 rounded-lg shadow-md mb-4">
              <h3 className="text-lg font-bold">{announcement.title}</h3>
              <p>{announcement.description}</p>
              <p className="text-sm text-gray-500">{new Date(announcement.date).toLocaleDateString()}</p>
              <a href={announcement.link} className="text-blue-500" target="_blank" rel="noopener noreferrer">
                View Link
              </a>
              <div className="mt-2">
                <button
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-lg w-11/12 md:w-1/3">
            <h2 className="text-xl font-semibold mb-4">{isEdit ? 'Edit Announcement' : 'Create Announcement'}</h2>
            <form onSubmit={handleSubmit}>
              ...
              <button type="button" onClick={() => setModalOpen(false)} className="bg-gray-300 text-black py-2 px-4 rounded mt-2 w-full">
                Cancel
              </button>
            </form>
          </div>
